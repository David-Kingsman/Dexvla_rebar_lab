...
def process_obs(obs, states, stats):
    ...
    traj_rgb_np = traj_rgb_np.astype(np.float32)
...
class qwen2_vla_policy:
    ...
    def process_batch_to_qwen2_vla(self, curr_image, robo_state, raw_lang):
        ...
        # New updated: convert to torch.Tensor
        if isinstance(curr_image, np.ndarray):
            curr_image = torch.from_numpy(curr_image).float().cuda()
        if isinstance(robo_state, np.ndarray):
            robo_state = torch.from_numpy(robo_state).float().cuda()  
...
def eval_bc(policy, deploy_env, policy_config, raw_lang=None, query_frequency=16):
    ...
    with open(stats_path, 'rb') as f:
        stats = pickle.load(f)
    # check stats state
    for k in ['qpos_mean', 'qpos_std', 'action_mean', 'action_std', 'action_max', 'action_min']:
        if k in stats:
            print(f"{k}: {stats[k]}")
        else:
            print(f"{k} not in stats!")
    ...
    for k in ['action_mean', 'action_std', 'action_max', 'action_min']:
        print(f"{k}: {stats[k]}, nan: {np.isnan(stats[k]).any()}, zero: {(stats[k]==0).any()}")
    #############################################################################################################
    ...
    for rollout_id in range(1000):
        ...
        with torch.inference_mode():
            ...
            for _ in range(2):
                ...
                batch = policy.process_batch_to_qwen2_vla(traj_rgb_np, robot_state, raw_lang)
            ...
            #     print("Warmup all_actions nan:", torch.isnan(all_actions).any())
            
            # for name, param in policy.policy.named_parameters():
            #     if "proj" in name or "policy_head" in name:
            #         print(f"{name} shape: {param.shape}, nan: {torch.isnan(param).any()}")
            time0 = time.time()
            for t in range(max_timesteps):
                ...
                robot_state = torch.from_numpy(robot_state).float().cuda()
                # print("robot_state after to tensor:", robot_state)
                # print("traj_rgb_np:", np.isnan(traj_rgb_np).any(), traj_rgb_np.shape, traj_rgb_np.dtype)
                # print("robot_state:", np.isnan(robot_state.cpu().numpy()).any(), robot_state.shape, robot_state.dtype)
                ...
                if t % query_frequency == 0:
                    ...
                    #     if hasattr(v, 'shape'):
                    #         print(f"batch[{k}] shape: {v.shape}, dtype: {v.dtype}, nan: {torch.isnan(v).any().item() if torch.is_floating_point(v) else 'N/A'}")
                    #     else:
                    ...
                    if policy_config['tinyvla']:
                        ...
                        chunks = torch.chunk(all_actions, chunks=all_actions.shape[1], dim=1)
                        # print("chunks lens:", len(chunks), "chunk shape:", chunks[0].shape)
                        action_queue.extend(chunks[0:query_frequency])
                        # print("action_queue length after extend:", len(action_queue))
                    # clear previous actions
                    while len(action_queue) > 0:
                        action_queue.popleft()
                    # action interpolation
                    # all_actions_cpu = all_actions.detach().cpu().to(torch.float32).numpy()
                    # # all_actions_cpu: shape (1, N, 7) or (N, 7)
                    # if all_actions_cpu.ndim == 3:
                    #     all_actions_cpu = all_actions_cpu[0]
                    # interp_actions = action_interpolation(all_actions_cpu, query_frequency)  # shape (1, query_frequency, 7)
                    # for i in range(query_frequency):
                    #     action_queue.append(torch.from_numpy(interp_actions[0, i]))
                    all_actions_cpu = all_actions.detach().cpu().to(torch.float32).numpy()
                    # ensure all_actions_cpu is 3D (B, T, D)
                    if all_actions_cpu.ndim == 2:
                        all_actions_cpu = all_actions_cpu[None, ...]  # (N, 7) -> (1, N, 7)
                    interp_actions = action_interpolation(all_actions_cpu, query_frequency)  # shape (1, query_frequency, 7)
                    for i in range(query_frequency):
                        action_queue.append(torch.from_numpy(interp_actions[0, i]))
                print("action_queue length:", len(action_queue))
                raw_action = action_queue.popleft()
                print("action_queue length:", len(action_queue))
                raw_action = raw_action.squeeze(0).cpu().to(dtype=torch.float32).numpy()
                print("raw_action nan:", np.isnan(raw_action).any(), "shape:", raw_action.shape)
                # import pdb; pdb.set_trace()
                ### 8. post process actions##########################################################
                action = post_process(raw_action)
                print("action after post_process nan:", np.isnan(action).any(), "shape:", action.shape)
                #####################################################
                print("after post_process action size:", action.shape)
                print('step {}, pred action: {}{}'.format(t, outputs, action))
                if len(action.shape) == 2:
                    action = action[0]
                if np.isnan(action).any():
                    print("Action contains nan, skip this step!")
                    continue
                ##### Execute ######################################################################
